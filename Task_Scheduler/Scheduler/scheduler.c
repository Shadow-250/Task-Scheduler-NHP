...
static volatile uint32_t g_ui32SchedulerTickCount; // Đếm stick từ 0 đến 2^32-1 sau đó quay lại 0
...
{
    g_ui32SchedulerTickCount++; // Systick timer trigger thì tăng 1
    if (RGB_TimerCount != 0) RGB_TimerCount--;//implement the timer
}
...
//
//*****************************************************************************
void SchedulerInit(uint32_t ui32TicksPerSecond)  // Input parameter systick in 1 second
{
...
//
//*****************************************************************************
void SchedulerRun(void)   // Consider every task in table
{
    uint32_t ui32Loop;
    tSchedulerTask *pi16Task; // pointer every task
    ...
    for(ui32Loop = 0; ui32Loop < g_ui32SchedulerNumTasks; ui32Loop++)
    {
        ...
        if(pi16Task->bActive &&(SchedulerElapsedTicksGet(pi16Task->ui32LastCall) >= pi16Task->ui32FrequencyTicks))
        {
            //
